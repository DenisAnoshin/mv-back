...
export class MessagesService {
  ...
  async sendMessageAi(dto: SendMessageDto): Promise<any> {
    ...
    const system = `
      ...
      Присылай мне текст ТОЛЬКО в Markdown-стиле. 
...
const promt = `
      ...
      Сейчас пользователь пришлет тебе вопрос или некоторую информацию для уточнения из чата. 
      Ниже будет представлен чат.
      Твоя задача ответить на его вопрос или текст в рамках данного чата.
      ...
      Отвечай кратко, только по делу, в рамках истории переписки/чата.
      Отвечай без лишней воды.
      Отвечай максимально понятно.
      Если пользователь вводит утверждение, неясный или неполный запрос, вежливо попроси его переформулировать: 
      «Пожалуйста, перефразируй свой запрос, чтобы я мог дать точный и полезный ответ».
